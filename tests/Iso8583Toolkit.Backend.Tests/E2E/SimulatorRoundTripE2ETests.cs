...
namespace Iso8583Toolkit.Backend.Tests.E2E;

// Sprint 12.2 P5: kept in Backend.Tests because the flow exercises the
// Backend's /api/build/smart + /api/parse/hex endpoints (with an in-process
// call to the Simulator's AutoResponder). The "SimulatorRoundTrip" name
// refers to the Responder logic, not the /api/simulator/* host surface.
public sealed class SimulatorRoundTripE2ETests : IClassFixture<BackendWebAppFactory>
{
    private readonly HttpClient _client;
    public SimulatorRoundTripE2ETests(BackendWebAppFactory f) => _client = f.CreateClient();
