...
using Iso8583Toolkit.Backend.OpenApi;
...
// SignalR + Simulator DI moved to Iso8583Toolkit.Agent in Sprint 12.2 P4.
// The Simulator lives in its own host process now — the Backend keeps only
// the utility/reference/workspace surface plus the frontend static assets.
...
// Agent-specific
// LocalSessionStore backs the workspace/templates surface + the /api/health
// TotalMessagesProcessed counter (which is Backend-local — after the split
// the Simulator flow logs into the Agent's own IMessageLog, so this counter
// only reflects what happens inside the Backend process).
